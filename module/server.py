...
class Server(object):
    def __init__(self, host='127.0.0.1', port=33333, timeout=100, client_nums=10):
        self.__host = host
        self.__port = port
    ...
    def handleMsg(self,data,s):
        ...
        if msgtype=='01':
            ...
            key='user'+str(random.randint(0,100))
            userdic={key:(name,passwd)}
            if self.r.exists(key):
                pass
            else:
                self.r.set(key,userdic)
            print 'send response'
            s.send('login success')
        else:
            pass
            ...
            for s in readable:
                if s is self.server:  # 是客户端连接
                    ...
                    if data:
                        ...
                        data1 = "%s %s say: %s" % (time.strftime("%Y-%m-%d %H:%M:%S"), self.client_info[s], data)
                        self.message_queues[s].put(data1)  # 队列添加消息
                        self.handleMsg(data,s)
